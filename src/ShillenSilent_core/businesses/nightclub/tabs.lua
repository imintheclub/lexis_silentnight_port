...
local function register(heistTab)
	...
	ui.button(gSafe, "nc_safe_unbrick", "Unbrick Safe", function()
		nc_logic.safe_unbrick()
	end)

	local gPop = ui.group(heistTab, "Popularity", nil, nil, nil, nil, "nightclub")
	ui.slider(gPop, "nc_popularity_value", "Popularity", 0, 1000, nc_logic.get_popularity_editor_value(), function(val)
		nc_logic.set_popularity_editor_value(val)
	end, nil, 10)
	ui.button(gPop, "nc_pop_apply", "Apply Popularity", function()
		nc_logic.apply_popularity_editor_value()
	end)
	ui.toggle(gPop, "nc_pop_lock", "Lock Popularity", nc_logic.get_popularity_lock_active(), function(val)
		nc_logic.set_popularity_lock_active(val)
	end)
